...
CREATE VIEW [dbo].[web40repwokrstationOOW60days]
AS
...
WHERE      (dbo.tblAssetCustom.Warrantydate < GETDATE() + 30) AND (dbo.tblAssetCustom.Warrantydate > GETDATE()) AND 
                        (dbo.tblComputersystem.Domainrole < 2) AND (dbo.tblAssetCustom.State = 1)
